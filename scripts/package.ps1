...
if (-not $Ucrt) {
    $Ucrt = @($env:GHCUP_MSYS2, 'C:\ghcup\msys64', 'C:\msys64') |
        Where-Object { $_ } |
        ForEach-Object { Join-Path $_ 'ucrt64' } |
        Where-Object { Test-Path $_ } |
        Select-Object -First 1
    if (-not $Ucrt) { throw 'no MSYS2 UCRT64 tree found; pass -Ucrt' }
}
Write-Host "UCRT64 tree: $Ucrt"
...
# WiX 7 refuses to run until the Open Source Maintenance Fee EULA is accepted (WIX7015).
# --acceptEula accepts it for this invocation alone and writes no acceptance file, which is what
# a build machine wants. The id is the major version: wix7 for the 7.0.0 pinned above.
wix build `
    --acceptEula wix7 `
    -d Version=$Version `
    -arch x64 `
